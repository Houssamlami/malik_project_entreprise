...
class AccountInvoice(models.Model):
    ...
    def get_total_colis_invoice(self):
        for record in self:
            colis = sum(self.env['stock.picking'].search([('partner_id', '=', self.partner_id.id),('state', '=', 'done'),('picking_type_id.code', '=', 'outgoing'),('origin', 'ilike', self.origin)]).mapped('total_colis_delivered')) - sum(self.picking_ids.filtered(lambda r: r.picking_type_id.code == 'incoming' and r.state == 'done').mapped('total_colis_delivered'))
...
class AccountInvoiceReport(models.Model):
    ...
    amount_total = fields.Float(string='Prix TTC', readonly=True)
