...
class ResPartner(models.Model):
    ...
    user_id = fields.Many2one(comodel_name='hr.employee', track_visibility='always')
    vendeur = fields.Many2one('hr.employee',related='team_id.vendeur', track_visibility='always')
    vendeur_commarcial = fields.Many2one(comodel_name='res.users', string="Commercial", track_visibility='always')
    Client_Volaille = fields.Boolean('Client Volailles', track_visibility='always')
    Client_Charcuterie =  fields.Boolean('Client Charcuterie', track_visibility='always')
    Client_GC = fields.Boolean(string='Client Gros Compte', track_visibility='always')
    Client_PC = fields.Boolean(string='Client Petit Compte', track_visibility='always')
    ...
    vat = fields.Char(string='VAT', track_visibility='always')
    pricelist_for_regroupby = fields.Many2one(comodel_name='product.pricelist',related='property_product_pricelist', store=True)
    Bolocagettm = fields.Many2one(comodel_name='blockage.blockage', track_visibility='always')
    blocagex_limite_nbr_fac=fields.Boolean('Blocage par Nombre de facture')
    limite_nbr_fac=fields.Integer('limite Nombre de facture', track_visibility='always')
    nbr_fac_ouverte=fields.Integer('Nombre de facture ouvertes',compute='_on_calcule_factures')
    blocagex_limite_credit=fields.Boolean('Blocage par limite de credit', track_visibility='always')
    bloque=fields.Boolean('bloque',compute='_on_change_credit',store=True)
    blocagex_limite_credit_charcuterie=fields.Boolean('Blocage par limite de credit charcuterie', track_visibility='always')
    blocagex_echeance_facture_charcuterie=fields.Boolean('Blocage par echeance charcuterie', track_visibility='always')
    limite_nbr_fac_charcuterie=fields.Integer('limite Nombre de facture charcuterie', track_visibility='always')
    ...
    limite_credit_charcuterie=fields.Float('limite credit charcuterie', track_visibility='always')
    limite_credit_volaille=fields.Float('limite credit  volaille', track_visibility='always')
